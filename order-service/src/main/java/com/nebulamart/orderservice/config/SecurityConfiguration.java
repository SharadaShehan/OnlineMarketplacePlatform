    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/order/courier/**").hasAuthority("ROLE_COURIER")
                )
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/order/seller/**").hasAuthority("ROLE_SELLER")
                )
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/order/customer/**").hasAuthority("ROLE_CUSTOMER")
                )
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/orders/**").authenticated()
                )
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/review/**").hasAuthority("ROLE_CUSTOMER")
                )
                .authorizeHttpRequests(authorize -> authorize
